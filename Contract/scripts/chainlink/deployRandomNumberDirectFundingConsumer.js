...
async function deployRandomNumberDirectFundingConsumer(chainId) {
    ...
    const seatAssignerV2Factory = await ethers.getContractFactory(
        "SeatAssigner"
    )
    const seatAssignerConsumerV2 = await seatAssignerV2Factory.deploy(
        linkAddress,
        wrapperAddress
    )
    ...
    await seatAssignerConsumerV2.deployTransaction.wait(waitBlockConfirmations)

    console.log(
        `Random Number Direct Funding Consumer deployed to ${randomNumberConsumerV2.address} on ${network.name}`
    )
    console.log(
        `Seat Assigner Random Number Direct Funding Consumer deployed to ${seatAssignerConsumerV2.address} on ${network.name}`
    )
    ...
    if (!developmentChains.includes(network.name) && process.env.ETHERSCAN_API_KEY) {
        ...
        await run("verify:verify", {
            address: seatAssignerConsumerV2.address,
            constructorArguments: [linkAddress, wrapperAddress],
        })
    }
    ...
    if (chainId == 31337) {
        ...
        console.log(
            `Run the following command to fund your Seat Assigner contract with 25,000 link tokens: npx hardhat transfer-link --recipient ${seatAssignerConsumerV2.address} --linkaddress ${linkAddress} --amount  25000000000000000000000 --network ${network.name}`
        )
    } else {
        console.log(
